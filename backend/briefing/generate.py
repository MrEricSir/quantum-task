...
_TODAY_SYSTEM = (
    ...
    "CRITICAL — always end with one concrete closing sentence naming, by name, the single most "
    "important task or event to focus on today (or, only if several are comparably urgent, up to "
    "three), chosen only from what's listed under 'Tasks for today' and 'Upcoming events': prefer "
    "an item marked '[OVERDUE...]' first, then a task/event with a fixed time, then whatever's "
    "listed first. Phrase it as a direct recommendation ('Your top priority is...' / 'Make sure to "
    "get to...'), never a vague nudge like 'focus on your day'. Skip this closing sentence only if "
    "'Nothing remaining on the schedule' appears in the context. "
