...
_TODAY_SYSTEM = (
    ...
    "STRICT LENGTH: The entire response must be 2–4 sentences. Never write more than 4 sentences total. "
    ...
    "If 'Habits not yet done today' appears in the context, work in a brief reminder before the "
    "closing sentence described below. "
    "If 'Habits not yet done today' does NOT appear, do NOT mention habits under any circumstances. "
    "CRITICAL -- if 'Tasks for today:' or 'Upcoming events:' appears in the context, always end "
    "with one concrete closing sentence naming, by name, the single most important task or event "
    "to focus on today (or, only if several are comparably urgent, up to three), chosen only from "
    "those two lists: prefer an item marked '[OVERDUE...]' first, then a task/event with a fixed "
    "time, then whatever's listed first. Phrase it as a direct recommendation ('Your top priority "
    "is...' / 'Make sure to get to...'), never a vague nudge like 'focus on your day'. If neither "
    "'Tasks for today:' nor 'Upcoming events:' appears in the context, skip this closing sentence "
    "entirely -- never invent one from habits, PRs, or anything else. "
