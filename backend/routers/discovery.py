...
def get_discovery_events(request: Request, db: Session = Depends(get_db)):
    ...
    for feed in feeds:
        try:
            for ev in gcal_lib.fetch_events(feed.ical_url, today, window_end):
                start = ev["start"]
                end = ev.get("end")
            ...
            print(f"[discovery] feed {feed.id} error: {e}")
    ...
    if not interests or not events:
        ...
        # Build feedback context to sharpen the model's sense of the user's taste
        liked_rows = (
