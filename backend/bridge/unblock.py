...
transitions it to "pending" (claimable) once its upstream job reaches "done", appending the
upstream job's completion note to the downstream job's prompt so the companion job reflects
what was actually built, not just the original plan.
...
def unblock_dependent_jobs(db: Session) -> list[models.BridgeJob]:
    """Transition any "blocked" job whose depends_on_job_id points at a now-"done" job to
    "pending", appending the upstream job's result to the downstream job's prompt. A job
    blocked on an upstream that ended in "error" or "stalled" is left blocked -- unblocking
    into a run against a broken/incomplete upstream needs a human decision, not an automatic
    one. Self-limiting like check_stale_bridge_jobs: once transitioned, a job no longer
    matches status == "blocked", so calling this repeatedly never re-processes the same job
    twice."""
    blocked_jobs = (
