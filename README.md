...
- **Health experiments**: a weekly experiment is generated automatically, targeting either a Withings metric (steps, body fat, weight), an incremental change to an established habit or workout routine (e.g. "row 2 mi/day instead of 1 mi/day"), or reducing/eliminating a food eaten regularly (e.g. "cut out coffee entirely this week", matched by exact food name -- logged foods aren't normalized/grouped, so near-duplicate phrasings of the same food are tracked separately); the app avoids repeating a recent experiment's target and evaluates outcomes when the experiment is dismissed -- workout-routine experiments get a real before/after comparison (with a significance badge), food experiments get a before/during frequency comparison (a plain count, not a significance test) and are checked for adherence before showing a weight/fat verdict (no credit given if the food wasn't actually cut back), and their weight/fat baseline is drawn from weeks the food was actually being eaten rather than an unmatched average of every other week, with a caveat surfaced if average calories also shifted >15% that week (a lightweight, single-variable confound check -- not a full multivariate model, which the app's typical data volume can't reliably support; see PRODUCT_NOTES.md) -- both shown alongside the correlation cards
