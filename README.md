...
- **Health experiments**: a weekly experiment is generated automatically, targeting either a Withings metric (steps, body fat, weight), an incremental change to an established habit or workout routine (e.g. "row 2 mi/day instead of 1 mi/day"), or reducing/eliminating a food eaten regularly (e.g. "cut out coffee entirely this week", matched by exact food name -- logged foods aren't normalized/grouped, so near-duplicate phrasings of the same food are tracked separately); every experiment gets a linked daily habit, auto-checked rather than manually tapped (a Withings-goal habit checks when the day's synced metric meets the goal, a workout-routine habit checks the moment a matching workout is logged, and a food-elimination habit checks off each trailing day the food goes unlogged, re-evaluated over a few trailing days to tolerate backdated food-log entries) -- the app avoids repeating a recent experiment's target and evaluates outcomes when the experiment is dismissed -- both workout-routine and food-elimination experiments are checked for adherence before showing a weight/fat verdict (no credit given if the routine wasn't actually changed, or the food wasn't actually cut back), their weight/fat baseline is drawn from weeks the routine/food was actually established rather than an unmatched average of every other week, and a caveat is surfaced if average calories also shifted >15% that week (a lightweight, single-variable confound check -- not a full multivariate model, which the app's typical data volume can't reliably support; see PRODUCT_NOTES.md) -- workout-routine experiments additionally get a real significance badge (a genuine before/after t-test) shown alongside the correlation cards; a "What's Working" summary on the Health page aggregates every dismissed experiment by the real-world routine it tested (a Withings metric, workout type, or food), across however many times you've tried it, into a per-routine verdict (worth prioritizing / didn't help / mixed / no clear signal yet) -- only weeks you actually adhered to the routine count toward it, and sample sizes are shown honestly (n=1 is common and treated as exactly that, not overclaimed as a trend)
- **Correlation scatter plots**: explore relationships between tracked metrics (e.g. steps vs. sleep, food quality vs. energy, daily sodium vs. water weight) -- each food log entry gets an LLM-estimated sodium value (same pattern as calories), correlated against Withings' hydration reading once enough weekly data exists (at least 3 paired weeks); this correlation is passive/exploratory only, not part of the auto-generated weekly Health Experiments below
...
Next to **▶ Run**, a **Branch** field shows the branch name the bridge would auto-generate (`qtask/<card id>-<slug of the title>`) as grayed-out placeholder text — type your own name to use it instead, or leave it blank to keep the default. Left blank, the name is still decided fresh when the bridge actually picks up the job (from whatever the card's title is *then*), not locked in at the moment you click Run. Clearing a name you've already typed goes right back to that same default — there's nothing else to click. Your own names don't need to start with `qtask/` — `--list`/`--switch`/`--cleanup` find the worktree either way.

The field stays editable while a job is queued or running, showing its real current name once one exists — edit it and click away to rename. For a job that hasn't started yet, that's applied the moment it picks up the job, same as setting it before you clicked Run. For one already running, the machine working it picks up the request the next time it checks in (a heartbeat every few minutes, not instant) and renames the actual git branch itself, confirming back to the app once done — the Branch field is just asking for that, not doing it directly. Once a job is done, errored, or stalled, the field locks — nothing is still running to pick up a rename, and resuming reuses the exact same branch/worktree by design. Renamed the branch by hand via raw git instead, and now the app's record is out of sync? Run `qtask-bridge --rename-branch new-name` from the worktree (or right after switching to it) — it renames the actual git branch and updates the app's record to match in one step.
...
| `got a massage` | Logs a wellness entry |
| `did a brain-training session` | Logs a mental exercise entry |
...
- **Trip retrospective** (no schedule to set — fires once, right when you flip the Travel mode toggle back off in Settings) — a "welcome back" digest covering the trip window: tasks completed, habit activity while away, health data logged, and confirmation habit streaks weren't broken by the trip. See [Habits](#habits)'s Trip mode entry.
